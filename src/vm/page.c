...
vm_supt_create (void)
{
  struct supplemental_page_table *supt = malloc(sizeof(struct supplemental_page_table));
...
vm_supt_lookup (struct supplemental_page_table *supt, void *page)
{
  ...
  if(!elem) return hash_entry(elem, struct supplemental_page_table_entry, elem);
  return NULL;
}
...
{
  struct supplemental_page_table_entry *spte = vm_supt_lookup(supt, page);
  if (!spte) PANIC("set dirty - the request page doesn't exist");

  if(!(spte->dirty)) spte->dirty=value;
  return true;
}
...
vm_load_page(struct supplemental_page_table *supt, uint32_t *pagedir, void *upage)
{
  ...
  if(spte->status == ON_FRAME) return true;
...
{
  struct supplemental_page_table_entry *spte = vm_supt_lookup(supt, page);
  if(!spte) PANIC ("munmap - some page is missing; can't happen!");
  ...
  case ON_FRAME:
    ...
    if(!is_dirty) is_dirty = pagedir_is_dirty(pagedir, spte->upage);
    if(!is_dirty) is_dirty = pagedir_is_dirty(pagedir, spte->kpage);
    if(is_dirty) file_write_at (f, spte->upage, bytes, offset);
  ...
  case ON_SWAP:
    {
      ...
      else vm_swap_free (spte->swap_index);
    }
    break;
...
vm_unpin_page(struct supplemental_page_table *supt, void *page)
{
  ...
  if (spte->status == ON_FRAME) vm_frame_unpin (spte->kpage);
}
...
spte_less_func(const struct hash_elem *a, const struct hash_elem *b, void *aux UNUSED)
{
  ...
  if(a_entry->upage < b_entry->upage) return true;
  else return false;
}
static void
spte_destroy_func(struct hash_elem *elem, void *aux UNUSED)
{
  ...
  else if(entry->status == ON_SWAP) vm_swap_free (entry->swap_index);
