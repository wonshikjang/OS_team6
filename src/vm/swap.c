...
void vm_swap_in (swap_index_t swap_index, void *page)
{
  ...
  if (bitmap_test(swap_available, swap_index)) PANIC ("Error, invalid read access to unassigned swap block");
...
vm_swap_free (swap_index_t swap_index)
{
  // check the swap region
  ASSERT (swap_index < swap_size);
  if (bitmap_test(swap_available, swap_index)) PANIC ("Error, invalid free request to unassigned swap block");
  bitmap_set(swap_available, swap_index, true);
}
