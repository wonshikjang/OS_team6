...
vm_frame_allocate (enum palloc_flags flags, void *upage)
{
  ...
  if (!frame_page) {
    ...
    bool is_dirty = false;
    is_dirty = pagedir_is_dirty(f_evicted->t->pagedir, f_evicted->upage);
    if(!is_dirty) is_dirty = pagedir_is_dirty(f_evicted->t->pagedir, f_evicted->kpage);
...
struct frame_table_entry* pick_frame_to_evict( uint32_t *pagedir )
{
  ...
  size_t it;
  for(it = 0; it <= 2*n; it++) // prevent infinite loop. 2n iterations is enough
  {
...
vm_frame_set_pinned (void *kpage, bool new_value)
{
  ...
  f_tmp.kpage = kpage;
  struct hash_elem *h = hash_find (&frame_map, &(f_tmp.helem));
  if (h == NULL) PANIC ("The frame to be pinned/unpinned does not exist");
...
static bool frame_less_func(const struct hash_elem *a, const struct hash_elem *b, void *aux UNUSED)
{
  ...
  if(a_entry->kpage < b_entry->kpage) return true;
  else return false;
}
