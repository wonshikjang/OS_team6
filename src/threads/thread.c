...
void move_thread_unblock(int64_t ticks){
	...
	while(!list_empty(&block_list)){
		struct thread *will_unblock = list_entry(list_front(&block_list), struct thread, elem);
		int64_t first_tick = will_unblock-> wakeup_tick;
		if(first_tick <= ticks){
...
bool compare_tick(const struct list_elem *x, const struct list_elem *y, void *aux UNUSED){
